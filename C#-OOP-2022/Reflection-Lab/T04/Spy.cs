namespace Stealer
{
    ...
    public class Spy
    {
        ...
        public string CollectGettersAndSetters(string investigatedClass)
        {
            ...
            //Output getters seperately from setters, for example 3 getters then 3 setters, not 1 get 1 set...
            foreach (var method in methods)
            {
                //Getters
                if (method.Name.StartsWith("get")) output.AppendLine($"{method.Name} will return {method.ReturnType}");
                
            }
            foreach (var method in methods)
            {
                //Setters
                if (method.Name.StartsWith("set")) output.AppendLine($"{method.Name} will set field of {method.GetParameters()[0].ParameterType}");
            }
